...
cik_input = st.text_input(
    "Enter Fund CIK (e.g., 1000045 for Berkshire Hathaway):", 
    value=DEFAULT_CIK, 
    max_chars=10
).strip()
            ...
            with st.spinner(f"Searching for latest 13F filing for CIK {cik_input}..."):
                ...
                if metadata:
                    ...
                    message_placeholder.success(
                        f"Found latest 13F filing for **{metadata['name']}** (CIK: {cik_input}).\n"
                        f"Accession Number: `{acc_num}` | Filing Date: **{metadata['filingDate']}**"
                    )
                else:
...
with col2:
    ...
    if st.session_state.filing_metadata:
        ...
        if st.button(f"2. Fetch Infotable Holdings (Acc: {acc_num[:10]}...)", use_container_width=True):
            with st.spinner(f"Fetching XML data for {acc_num}... This may take a moment."):
                try:
    ...
    if st.button("3. Insert All Holdings into Database", type="primary", use_container_width=True):
        with st.spinner("Inserting data into SQLite database..."):
            try:
                ...
                # Clear the session state to prepare for the next import
                st.session_state.filing_df = pd.DataFrame()
                st.session_state.filing_metadata = {}
            except Exception as e:
